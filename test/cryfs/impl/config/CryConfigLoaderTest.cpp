...
namespace boost {
    ...
    static inline ostream &operator<<(ostream &stream, const CryConfigFile &) {  // NOLINT(misc-use-anonymous-namespace)
...
namespace cryfs {
  ...
  // does not follow the implicit using-directive of a nested unnamed namespace. Moving it
  // into one would silently stop it being found, so static is the right mechanism here.
  static inline ostream &operator<<(ostream &stream, const CryConfigLoader::ConfigLoadResult &) {  // NOLINT(misc-use-anonymous-namespace)
