...
fn file_in_rootdir(test_driver: impl TestDriver) -> impl TestReady {
    test_driver
        .create_filesystem()
        .setup(async |fixture| {
            // First create a file so we have something to change ownership for
            fixture
                .filesystem
        ...
        .test(async |fixture, file| {
            fixture
                .filesystem
                .chown(Some(file), Some(Uid::from(1000)), Some(Gid::from(1000)))
                .await
                .unwrap();
        })
        .expect_op_counts(|fixture_type, _atime_behavior| ActionCounts {
            blobstore: BlobStoreActionCounts {
                ...
                blob_read_all: match fixture_type {
                    ...
                    FixtureType::Fusemt | FixtureType::FuserWithoutInodeCache => 1, // TODO Why more than fusemt? Maybe because our CryNode structs don't cache the node and only store the path, so we have to lookup for fuser and then lookup everythin again?
                },
...
fn dir_in_rootdir(test_driver: impl TestDriver) -> impl TestReady {
    test_driver
        .create_filesystem()
        .setup(async |fixture| {
            // First create a directory so we have something to change ownership for
            fixture
                .filesystem
        ...
        .test(async |fixture, dir| {
            fixture
                .filesystem
                .chown(Some(dir), Some(Uid::from(1000)), Some(Gid::from(1000)))
                .await
                .unwrap();
        })
        .expect_op_counts(|fixture_type, _atime_behavior| ActionCounts {
...
fn symlink_in_rootdir(test_driver: impl TestDriver) -> impl TestReady {
    test_driver
        .create_filesystem()
        .setup(async |fixture| {
            // First create a symlink so we have something to change ownership for
            fixture
                .filesystem
                .create_symlink(
                    None,
        ...
        .test(async |fixture, symlink| {
            fixture
                .filesystem
                .chown(Some(symlink), Some(Uid::from(1000)), Some(Gid::from(1000)))
                .await
                .unwrap();
        })
        .expect_op_counts(|fixture_type, _atime_behavior| ActionCounts {
            ...
            low_level: LLActionCounts {
                ...
                load: match fixture_type {
                    FixtureType::FuserWithInodeCache => 1,
                    FixtureType::FuserWithoutInodeCache | FixtureType::Fusemt => 2,
                },
                store: 1,
                ..LLActionCounts::ZERO
            },
