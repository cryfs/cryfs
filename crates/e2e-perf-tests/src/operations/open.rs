            ...
            ActionCounts {
                blobstore: BlobStoreActionCounts {
                    ...
                    store_load: match fixture_type {
                        FixtureType::FuserWithInodeCache => close_after, // If the inode is already cached, opening a file doesn't need to do any ops
                ...
                high_level: HLActionCounts {
                    ...
                    blob_data: match fixture_type {
                        FixtureType::FuserWithInodeCache => 7 * close_after, // If the inode is already cached, opening a file doesn't need to do any ops
                        FixtureType::Fusemt => 9 + 7 * close_after,
                        FixtureType::FuserWithoutInodeCache => 16 + 14 * close_after, // TODO Why more than fusemt? Maybe because our CryNode structs don't cache the node and only store the path, so we have to lookup for fuser and then lookup everythin again?
                    },
            ...
            ActionCounts {
                blobstore: BlobStoreActionCounts {
                    ...
                    blob_read_all: match fixture_type {
                        ...
                        FixtureType::FuserWithoutInodeCache => 3 + 1 * close_after, // TODO Why more than fusemt? Maybe because our CryNode structs don't cache the node and only store the path, so we have to lookup for fuser and then lookup everythin again?
                    },
                    blob_read: match fixture_type {
                        FixtureType::FuserWithInodeCache => close_after, // If the inode is already cached, opening a file doesn't need to do any ops
                        FixtureType::Fusemt => 2 + close_after,
                ...
                high_level: HLActionCounts {
                    ...
                    blob_data: match fixture_type {
                        FixtureType::FuserWithInodeCache => 7 * close_after, // If the inode is already cached, opening a file doesn't need to do any ops
                        FixtureType::Fusemt => 18 + 7 * close_after,
                        FixtureType::FuserWithoutInodeCache => 34 + 23 * close_after, // TODO Why more than fusemt? Maybe because our CryNode structs don't cache the node and only store the path, so we have to lookup for fuser and then lookup everythin again?
                    },
                    store_flush_block: close_after,
                    ..HLActionCounts::ZERO
                },
            ...
            ActionCounts {
                blobstore: BlobStoreActionCounts {
                    ...
                    blob_read_all: match fixture_type {
                        ...
                        FixtureType::FuserWithoutInodeCache => 7 + 5 * close_after, // TODO Why more than fusemt? Maybe because our CryNode structs don't cache the node and only store the path, so we have to lookup for fuser and then lookup everythin again?
                    },
                    blob_read: match fixture_type {
                        FixtureType::FuserWithInodeCache => close_after, // If the inode is already cached, opening a file doesn't need to do any ops
                        FixtureType::Fusemt => 4 + close_after,
                ...
                high_level: HLActionCounts {
                    ...
                    blob_data: match fixture_type {
                        FixtureType::FuserWithInodeCache => 7 * close_after, // If the inode is already cached, opening a file doesn't need to do any ops
                        FixtureType::Fusemt => 36 + 7 * close_after,
                        FixtureType::FuserWithoutInodeCache => 70 + 59 * close_after, // TODO Why more than fusemt? Maybe because our CryNode structs don't cache the node and only store the path, so we have to lookup for fuser and then lookup everythin again?
                    },
                    store_flush_block: close_after,
                    ..HLActionCounts::ZERO
                },
