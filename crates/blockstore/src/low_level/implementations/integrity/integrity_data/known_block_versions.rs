...
use crate::BlockId;
use cryfs_utils::{
    binary::{BinaryReadExt, BinaryWriteExt, read_nonzerou32, write_nonzerou32},
    safe_panic,
};
...
impl<'a> Drop for BlockVersionTransaction<'a> {
    fn drop(&mut self) {
        if self.0.is_some() {
            safe_panic!(
                "Active BlockVersionTransaction left scope. Please make sure you call commit() or cancel() on it."
            );
        }
    }
}
