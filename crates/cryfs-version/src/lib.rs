    ...
    () => {{
        {
            ...
            const RESULT: $crate::Version<&'static str> = $crate::Version {
                major: match u32::from_str_radix(env!("CARGO_PKG_VERSION_MAJOR"), 10) {
                    Ok(v) => v,
                    Err(_) => panic!("Invalid CARGO_PKG_VERSION_MAJOR"),
                },
                minor: match u32::from_str_radix(env!("CARGO_PKG_VERSION_MINOR"), 10) {
                    Ok(v) => v,
                    Err(_) => panic!("Invalid CARGO_PKG_VERSION_MINOR"),
                },
                patch: match u32::from_str_radix(env!("CARGO_PKG_VERSION_PATCH"), 10) {
                    Ok(v) => v,
