...
*/
        ...
        protected:
            // guard with !std::is_same<IdValueType, std::decay_t<U>> to make sure it doesn't hide copy or move constructor
            template <class U, std::enable_if_t<!std::is_same<IdValueType, std::decay_t<U>>::value && std::is_constructible<UnderlyingType, U>::value, int> = 0>
