...
use crate::blockstore::{
    high_level::{LockingBlockStore, RemoveResult, TryCreateResult},
    low_level::{BlockStore, BlockStoreDeleter, BlockStoreReader, BlockStoreWriter},
    BlockId,
};
use crate::data::Data;
use crate::utils::async_drop::{AsyncDrop, AsyncDropGuard, SyncDrop};
...
impl<B: BlockStore + Send + Sync + Debug + 'static> BlockStoreDeleter for BlockStoreAdapter<B> {
    async fn remove(&self, id: &BlockId) -> Result<crate::blockstore::low_level::RemoveResult> {
...
impl<B: BlockStore + Send + Sync + Debug + 'static> BlockStoreWriter for BlockStoreAdapter<B> {
    async fn try_create(
        ...
        id: &BlockId,
        data: &[u8],
    ) -> Result<crate::blockstore::low_level::TryCreateResult> {
        self.0.try_create(id, &data.to_vec().into()).await.map(|r| match r {
            TryCreateResult::SuccessfullyCreated => crate::blockstore::low_level::TryCreateResult::SuccessfullyCreated,
            TryCreateResult::NotCreatedBecauseBlockIdAlreadyExists => crate::blockstore::low_level::TryCreateResult::NotCreatedBecauseBlockIdAlreadyExists,
        })
    }
