...
namespace cpputils
{
  ...
  public:
    ...
    template <size_t size>
    FixedSizeData<size> take() const;
    template <size_t size>
    FixedSizeData<SIZE - size> drop() const;

    const std::array<unsigned char, BINARY_LENGTH> &as_array() const
    {
      return _data;
    }
  ...
  template <size_t SIZE>
  constexpr size_t FixedSizeData<SIZE>::BINARY_LENGTH;
  template <size_t SIZE>
  constexpr size_t FixedSizeData<SIZE>::STRING_LENGTH;
